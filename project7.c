...
void *myalloc(int size)
{
    ...
    while (current != NULL)
    {
        ...
        if (current->in_use == 0)
            if (current->size >= (int)(PADDED_SIZE(size) ))

            {
                current->in_use = 1;

                if (current->size > (int)(PADDED_SIZE(size) + PADDED_SIZE(sizeof(struct block)))+PADDED_SIZE(1))
                {
...
{
    struct block *temp_block = PTR_OFFSET(pointer, -PADDED_SIZE(sizeof(struct block)));
    temp_block->in_use = 0;
}
